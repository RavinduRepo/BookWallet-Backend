...
const signUp = async (req, res) => {
    try {
        ...
        const user = await authService.signUp(username, email, password, description);
...
const signIn = async (req, res) => {
    try {
        ...
        if (user) {
            res.status(200).json({ message: 'User signed in successfully', token, userId: user.user_id, username: user.username, email: user.email, description:user.description });
        } else {
